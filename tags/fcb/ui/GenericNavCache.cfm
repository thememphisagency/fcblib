...
<!--- Vertical Navigation Menu (with caching and uses externalURL to determined if dmNavigation should open in a new window --->
...
<cfset qNav = evaluate("o."&attributes.functionMethod&"(objectid=attributes.navID, lColumns='externallink,lNavIDAlias,externalURL', "&attributes.functionArgs&", afilter=navFilter)")>
...
//				}
				...
				// check to see if the extrnalURL is being used
				if(len(qNav.externalURL[i])){
					href= qNav.externalURL[i];
					target = '_blank';				
				}
				...
				// write the link
				writeOutput("><a href="""&href&""" target="""&target&""">"&trim(qNav.ObjectName[i]) & "</a>");
			}
		}
	}
