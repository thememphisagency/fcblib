...
<cfcomponent extends="farcry.plugins.farcrycms.packages.types.dmLink" displayname="Link" bSchedule="true" bUseInTree="false" bFriendly="true" bObjectBroker="true" bPaginatorSearchByCategory="1" bPaginatorAnimateByTypeRule="1">
  ...
  <cfproperty ftSeq="6" ftfieldset="Link Information" name="readMoreText" ftlabel="Read More Text" type="nstring" hint="Display text for read more link for object teasers" required="no" default="Read more">
	<cfproperty ftSeq="7" ftfieldset="Sensitive Data?" ftLabel="Sensitive" name="bSensitive" type="boolean" hint="" required="no" default="0" ftRenderType="checkbox" />


	<!--- 	Fix the checkPermission error. This occurs because bUseInTree is set to true for this type
			as it needs to display in the site tree --->
	<cftry>
		<cfset application.navid.dmLink = application.fc.utils.createJavaUUID() />

		<cfcatch>
			<!--- this is to stop errors on install --->
		</cfcatch>
	</cftry>
