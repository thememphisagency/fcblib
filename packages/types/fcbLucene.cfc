...
|| DESCRIPTION || 
$Description: fcbLucene Type 
Configuration object for Lucene Search Collections
$

|| DEVELOPER ||
$Developer: Geoff Bowers (modius@daemon.com.au) $
--->
<cfcomponent extends="farcry.core.packages.types.types" displayname="Lucene Collection" hint="Configuration object for Lucene free text search collection." bSchedule="false" bFriendly="false" bSystem="true">
<!------------------------------------------------------------------------
type properties
------------------------------------------------------------------------->	
...
<cfproperty ftseq="4" ftfieldset="Collection Details" name="collectionname" type="string" hint="Lucene/ColdFusion collection name." required="no" default="" ftlabel="Collection Name" ftdisplayonly="true" />
...
lucene maintenance methods (maybe move to luceneservice.cfc)
---------------------------
...
<cffunction name="delete" access="public" hint="Delete associated collection and content item." returntype="struct" output="false">
	...
	<cfset var oLucene=createobject("component", "farcry.plugins.fcblib.packages.custom.lucene") />
	<cfscript>
		oLucene.init();
	</cfscript>
	...
	<cfif stReturn.bSuccess AND arguments.bDeleteCollection>
		<cfset stConfig=createobject("component", "farcry.plugins.fcblib.packages.types.fcbLucene").getData(objectid=objectid) />
...
<cffunction name="getIndexStrings" access="public" hint="Get list of all indexable string properties for a specific content type." output="false" returntype="string">
	...
	FROM qListData
	WHERE datatype IN ('string','nstring','longchar','boolean')
	</cfquery>
...
<cffunction name="getIndexProperties" access="private" hint="Get query of all indexable properties for a specific content type." output="false" returntype="query">
	...
	<cfif NOT structkeyexists(application.stcoapi, arguments.typename)>
		<cfthrow type="Application" errorcode="plugins.fcbLucene.packages.types.fcbLucene" message="Typename (#arguments.typename#) is invalid." detail="The typename must be available in the application in order to build a collection." />
	</cfif>
	
	<cfloop collection="#application.stcoapi[arguments.typename].stProps#" item="prop">
		<cfif ListFindNoCase("string,nstring,longchar,date,boolean", application.stcoapi[arguments.typename].stProps[prop].metadata.type)>
			<cfset queryAddRow(qListData) />
	...
	FROM qListData
	WHERE property NOT IN ('displayMethod','status','commentlog','ownedby','createdby','lockedBy','lastupdatedby', 'locked')
	ORDER BY property
	</cfquery>
