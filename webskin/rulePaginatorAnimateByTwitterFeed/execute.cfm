...
<cfimport taglib="/farcry/core/tags/formtools" prefix="ft" />
	...
	<cfif arrayLen(q) GT 0>
		...
		<cfif stObj.bArchive>
		    ...
		    <script type="text/javascript">
				...
				<cfif NOT structKeyExists(request, 'bRulePaginatorAnimateByType')>
					...
					function nextScreen(obj){
						...
						if(obj.sliderDirection == 'horizontal'){
							//Since we are on the last screen, let's break out of the function
					...
					function updatePaginatorState(obj){						
						...
						//Next State	
						if(obj.iSlidingWidth + obj.iItemWidth >= obj.iTotalWidth){							
							jQuery(obj.divId).find('.nextAnnouce').removeClass('nextActive');
							jQuery(obj.divId).find('.nextAnnouce').addClass('nextInactive'); 
						}
						else{
							jQuery(obj.divId).find('.nextAnnouce').removeClass('nextInactive');
							jQuery(obj.divId).find('.nextAnnouce').addClass('nextActive');	
						}											
					}
			...
			<cfset iEndRow = arrayLen(q) />
		<cfelse>	
			<cfset iEndRow = stObj.numItems />
		</cfif>
		
		<cfoutput>
			<div id="#sRuleId#" class="paginatedContent noBlockClick">
				<div class="header">
